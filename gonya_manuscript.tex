...
\hline
  \multicolumn{6}{| c |}{Blast2GO annotation}\\
\hline
...
\newpage
\section{Discussion}
This study presents a pipeline designed to identify single copy genes in transcriptomes from taxa with extensive parology, extracting and aligning single copy gene sequences and running phylogenetic inference. 
The pipeline was tested on the order gonyaulacales, phylum donphyceales, with publically available transcriptomes as well as transcriptomes characterised in this study. 
With publically available datasets such as the MMETSP project for marine microbes \cite{keeling2014marine}, previous data based limitations for exploring the species evolution within these abundant organsisms can now be overcome. 
However, the next obstacle lies in the methodology for investigating these taxa. 
Due to the size of their genomes, ancient lineages and extensive parology, transparent, reproducible and sound reasoning for gene slection is necessary. 
This pipeline is publically available at zenodo? as well as the single copy genes used to infer the species evolution in this study, as well as the log files for the *BEAST runs. 
...
1) infering species evolution from a small subset of genes such as ribosomal genes, whose evolution may not reflect that of the species; 
2) concatenating the genes selected and using maximum likelihood methods for species inference; 
3) selection of paralogs to infer species evolution; and 
4) transcriptome assemblies that are not as representative of biology as could be.
