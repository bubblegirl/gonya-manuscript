...
This study presents a set of scripts that take RNA-seq libraries, trims and assembles them, then searches for a set of single copy genes common to input transcriptomes which are then aligned and ready for phylogenetic inference. 
%TODO sentence on result
...
%TODO discuss the purely data driven approach not agreeing with morphology, incl. ref FBD-*beast findings
...
With publicly available libraries such as the MMETSP project for marine microbes \cite{keeling2014marine}, previous data based limitations for exploring the species evolution within these abundant organisms can now be overcome. 
...
In particular and highly relevant to working with dinoflagellate phylogenetics, the process of paralog elimination as input for species inference presented here is transparent and, the authors hope, reproducible with rudimentary programming skills. 
The gonyaulacales were chosen to test the methodology presented here due to the size of their genomes and extensive paralogs.
The scripts which for the basis of this study are publicly available through github and the single copy genes used to infer the species evolution in this study, as well as the log files for the *BEAST runs, are available on zenodo.

\subsection*{Assessment of methods tested in this study}
%TODO work out which parts of the following sections are still relevant
%TODO where do I put LBA and ILS sections?
...
To address this problem, trimming and assembly of RNA-seq libraries using Trimomatic and Trinity respectively are integral to the transcriptome assembly.
...
% overparamiterization as well as undfer parameterization cause mis-secification under BI, if not n thwe Felsenstein zone. However under-par has more severe impacts in giving the wrong species tree \cite{lemmon2004importance}
